...
class SypexDumper {
	...
	const INC = '.sx/';
	public $name = 'Sypex Dumper Pro 2.1';
	...
	public function __construct() {
		...
		if(V_PHP < 50503) trigger_error('PHP 5.5+ only', E_USER_ERROR);
		define('C_DEFAULT', 1);
		define('C_RESULT', 2);
		define('C_ERROR', 3);
		define('C_WARNING', 4);
...
//	print_r($args);exit;
		if ($args) { // консольный режим
		    ...
		    $this->l = l();
			$h = str_replace("\x77w\x77.", '', isset($_SERVER['HTTP_HOST']) ? getenv("HTTP_HOST") : getenv("HOSTNAME"));
			if($this->cron_mode || (!empty($this->l) && ($this->l['h'] == $h || preg_match('/^\w+$/', $h)))){}
			else {echo I('PHNjcmlwdD5hbGVydCgnTGljZW5zZSBub3QgZm91bmQuJyk7PC9zY3JpcHQ+');}
...
//		if(mysql_connect($this->CFG['my_host'] . ($this->CFG['my_host']{0} != ':' ? ":{$this->CFG['my_port']}" : ''),  $this->CFG['my_user'], $this->CFG['my_pass'])) {
			...
			else {define('V_MYSQL', 0); echo I('PHNjcmlwdD5hbGVydCgnTGljZW5zZSBub3QgZm91bmQuJyk7PC9zY3JpcHQ+');}
		}
		else {
			//echo mysql_error();
			define('V_MYSQL', 0); 
...
//		@fastcgi_finish_request();
	...
	protected function addRestoreJob($job) {
		...
		$filter = $object = array();
   		$this->createFilters($this->JOB['obj'], $filter, $object);
			...
			else {echo I('PHNjcmlwdD5hbGVydCgnTGljZW5zZSBub3QgZm91bmQuJyk7PC9zY3JpcHQ+');exit;}
			...
			foreach($list AS $item){
				switch($t){
					case 'TA':
						...
						else {$todo[] = array('TA', $item[0], 'SKIP'); continue;}
	...
	protected function runRestoreJob($continue = false){
		...
		$this->query = $query = $this->JOB['savesql'] ? [this, 'save_query'] : [$this->mysql, 'query']; // ADDED 2.0.7 PRO
...
function l(){
    $temp = '';
    if(file_exists(I('c3hkLmxpYy5waHA'))) include(I('c3hkLmxpYy5waHA')); else return false;
...
function I($s){$I = "\x62as\x6564\x5fde\x63od\x65"; return $I($s);}
